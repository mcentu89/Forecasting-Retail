...
def calidad_datos(x):
    ...
    temp.loc[x['event_name_2'].isna(),'event_name_2'] = 'Sin_evento'
...
def crear_variables(x):
    ...
    x_unido.drop(columns = ['sell_price','rotura_stock_3','rotura_stock_7','rotura_stock_15'],inplace=True)
...
def transformar_variables(x,y=None,modo = 'entrenamiento'):
    ...
    '''
    Esta función sirve tanto para entrenamiento como para ejecución:
    ...
    * Haciendo que el parámetro y sea opcional, ya que en ejecución no se usa
    ...
    Cuando se usa en modo entrenamiento se aplica el método fit_transform y guarda los objetos.
    ...
    #ONE HOT ENCODING
    var_ohe = ['event_name_1','event_name_2']
    ...
    x = x.drop(columns=['event_name_1','event_name_2','month','wday','weekday'])
...
def lanzar_ejecucion(df, modo = 'datos'):
    
    '''
    Esta función hace el forecast para cada producto, pero solo de un día.

    Recibe el nuevo dataset a predecir, que tiene que tener la estructura del excel que descargarás de la plataforma.

    Va recorriendo cada producto, cargando su modelo corresopondiente, seleccionando sus datos, y haciendo las predicciones.
    ...
    for cada in range(0,len(lista_modelos)):
        ...
        x = transformar_variables(x, y, modo = 'ejecucion')
        ...
        #Genera graficos de predicho vs real
            ...
            predicciones.plot(figsize = (12,4), title = producto);
...
def forecast_recursivo(x):
    ...
    Recibe el dataset a predecir, que tiene que tener siempre la estructura del excel que descargarás de la plataforma ya que para aplicar la recursividad:

    * Va a predecir el primer día para el cual tenga toda la información (es decir 15 días del día más antiguo)
    ...
    * Por tanto en la siguiente interacción va a predecir el siguiente día.

    Por ejemplo:

    Si el día más antiguo del dataset es el 09/12/2015 entonces el primer día que puede predecir (y del cual ya no tenemos dato) es el 24/12/2015.
    ...
    for cada in range(0,8):
        ...
        variable_productos = paso2_df.producto.values
